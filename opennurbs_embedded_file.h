...
public:
  ...
  const ON_EmbeddedFile& operator = (const ON_EmbeddedFile&);
  ...
  ON_wString Filename(void) const;
  ...
  bool LoadFromBuffer(ON_Buffer& buf);
  ...
  // Returns true if successful, else false.
  bool SaveToBuffer(ON_Buffer& buf) const;
