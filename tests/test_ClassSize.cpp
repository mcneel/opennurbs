...
TEST(ONTest, ONTestClassSizeON_ComponentStatus) {
  ...
  size_t sz = sizeof(ON_ComponentStatus); //2

  //RH-78870 - Was changed from V6 and originally 1 but because of size alignment does not break the SDK
  EXPECT_EQ(sz, 2);
...
TEST(ONTest, ONTestClassSizeON_WindowsDWriteFontInformation) {
  ...
  size_t sz = sizeof(ON_WindowsDWriteFontInformation); //856

  //RH-78870 - Shouldn't ever be used by a client and was only in 6 for prototyping so not considered a break
  EXPECT_EQ(sz, 856);
...
TEST(ONTest, ONTestClassSizeON_PhysicallyBasedMaterial) {
  ...
  size_t sz = sizeof(ON_PhysicallyBasedMaterial); //72

  //RH-78870 - Not a break since a pointer to it is all that's ever returned
  EXPECT_EQ(sz, 72);
